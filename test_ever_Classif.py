        ...
        # === STATE: COLLECTING - Analyzing for 5 seconds ===
            ...
            if elapsed >= collection_duration:
                if prediction_counts:
                    ...
                    print(f"🗑️  BIN CATEGORY:  {final_bin_category}")
                    print(f"📊 Confidence: {final_confidence:.1f}%")
                    print("="*50 + "\n")

                state = SHOWING_RESULT

        # === STATE: SHOWING_RESULT - Display final decision + bin category ===
        elif state == SHOWING_RESULT:
            # Outer box colored by bin category
            cv2.rectangle(frame, (30, 60), (610, 420), (0, 0, 0), -1)
            cv2.rectangle(frame, (30, 60), (610, 420), final_bin_color, 5)

            # "RESULT" header
            cv2.putText(frame, "RESULT", (240, 110),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 3)

            # Item name
            font_scale = 1.0 if len(final_decision) < 20 else 0.75
            cv2.putText(frame, final_decision, (60, 190),
                        cv2.FONT_HERSHEY_SIMPLEX, font_scale, (200, 200, 200), 2)

            # Confidence
            cv2.putText(frame, f"Confidence: {final_confidence:.1f}%", (160, 240),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)

            # Divider line
            cv2.line(frame, (60, 260), (580, 260), final_bin_color, 2)

            # BIG bin category label
            bin_font_scale = 1.8 if len(final_bin_category) <= 7 else 1.3
            text_size = cv2.getTextSize(final_bin_category, cv2.FONT_HERSHEY_SIMPLEX, bin_font_scale, 4)[0]
            text_x = (640 - text_size[0]) // 2
            cv2.putText(frame, final_bin_category, (text_x, 340),
                        cv2.FONT_HERSHEY_SIMPLEX, bin_font_scale, final_bin_color, 4)

            cv2.putText(frame, "Press SPACEBAR for next", (130, 400),
