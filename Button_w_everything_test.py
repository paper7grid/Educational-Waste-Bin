...
try:
    while True:
        raw   = camera.capture_array()              # XBGR format from camera
        frame = raw[:, :, 1:]                        # drop X → BGR for OpenCV display
        frame_count += 1
        H, W = frame.shape[:2]
        ...
        # ── COLLECTING ────────────────────────────────────────────
            ...
            if frame_count % 10 == 0:
                img = cv2.resize(frame, (224, 224))              # resize BGR frame
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)       # convert to RGB for model
                ...
                # Debug: show top 3 guesses in terminal each frame
                top3 = np.argsort(preds)[::-1][:3]
                print(f'  Top: {labels[top3[0]]} {preds[top3[0]]*100:.1f}%  |  '
                      f'{labels[top3[1]]} {preds[top3[1]]*100:.1f}%  |  '
                      f'{labels[top3[2]]} {preds[top3[2]]*100:.1f}%')

                # Only count frames where confidence is high enough
                ...
                if conf >= CONFIDENCE_THRESHOLD:
                    ...
                    print(f'  (skipped — {conf:.1f}% below {CONFIDENCE_THRESHOLD}% threshold)')

            if elapsed >= collection_duration:
                if not confidence_totals:
                    # Nothing passed threshold — just use raw best guess
                    print('  (no frame passed threshold, using raw best)')
                    preds_last = interpreter.get_tensor(output_details[0]['index'])[0]
                    best = int(np.argmax(preds_last))
                    final_confidence = float(preds_last[best] * 100)
                else:
